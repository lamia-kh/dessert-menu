...
function renderCartItem(name, quantity, price) {
...
function updateCartDisplay() {
  cartItems.innerHTML = '';
  products_incart.forEach(item => {
    renderCartItem(item.name, item.quantity, item.price);
  });
}

function addToCart(index){
  const item = products[index]
  const exis_item = products_incart.find(hitem => hitem.name === item.name)
  if (exis_item) {
    exis_item.quantity ++
    updateCartDisplay()
  } else {
    products_incart.push({name : item.name, quantity :1 ,price : item.price
    })
    item.quantity ++
    renderCartItem(item.name,1,item.price)
}}
